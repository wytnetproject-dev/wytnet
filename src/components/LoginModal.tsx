...
export default function LoginModal({ isOpen, onClose, onLoginSuccess, activeProduct }: LoginModalProps) {
  ...
  const productLabel = activeProduct === 'wytpass' ? 'WytPass' : 'WytSaaS';
  const backendPort = activeProduct === 'wytpass' ? '8001' : '8000';
  const backendUrl = `http://localhost:${backendPort}/auth/login`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Attempt backend call to the product's specific backend
      ...
      if (response.ok) {
        ...
        // Extract a username prefix as display name
      ...
      // Backend is offline, trigger mock fallback with descriptive status
      console.warn(`${productLabel} backend connection failed, falling back to mock authentication`, err);
      setStatusMessage(`${productLabel} backend (port ${backendPort}) offline. Logging in via mock fallback...`);
      
      setTimeout(() => {
        // Mock credentials successful log in
        onLoginSuccess('mock-jwt-token-xyz', email, `${productLabel} User`);
        setIsLoading(false);
        onClose();
      }, 1200);
      return;
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-3xl border border-white/20 bg-white/90 p-8 shadow-[0_20px_50px_rgba(0,102,204,0.15)] backdrop-blur-xl animate-fadeIn">
        ...
        <div className="text-center mb-8 relative z-10">
          ...
          <h2 className="text-xl font-extrabold text-wytnet-dark">
            Welcome back to {productLabel}
          </h2>
          <p className="text-xs font-semibold text-slate-400 mt-1">
            Access your secure {productLabel} dashboard
          </p>
        </div>
        ...
        {/* Extra helper credentials tooltip for developer convenience */}
