...
import { FileText, ExternalLink, Zap, Play, Search, Settings, ArrowUpRight, Plus, Link2, Code2, Terminal } from 'lucide-react';
import { Card, CardContent } from '@mui/material';

export default function WytPassDashboard() {
    ...
    <div className="flex-grow bg-[#f8fafc] overflow-y-auto px-8 py-6 select-none space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button className="flex items-center gap-2 bg-wytnet-blue hover:bg-wytnet-blue-light transition-all text-xs font-bold text-white px-4 py-2 rounded-lg cursor-pointer shadow-md hover:shadow-lg">
            ...
            return (
              <button
                ...
                className={`text-xs font-bold px-3 py-1.5 rounded-lg transition-all relative cursor-pointer ${
                  isActive
                    ? 'text-wytnet-blue bg-blue-50/50'
                    : 'text-slate-500 hover:text-wytnet-dark'
                }`}
              >
                {tab.label}
                {isActive && (
                  <span className="absolute bottom-[-11px] left-0 right-0 h-[2px] bg-wytnet-blue rounded-full"></span>
                )}
              </button>
            );
          })}
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs font-bold transition-all ${
                      isActive
                        ? 'bg-blue-50/50 text-wytnet-blue border-l-2 border-wytnet-blue rounded-l-none'
                        : 'text-slate-500 hover:text-wytnet-dark hover:bg-slate-50/50'
                    }`}
                  >
                    <span>{item.label}</span>
                    <span className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded ${
                      isActive ? 'bg-wytnet-blue text-white' : 'bg-slate-100 text-slate-400'
                    }`}>
                      {item.count}
                    </span>
                  </button>
              ...
              {[
                ...
                return (
                  <a
                    ...
                    className="w-full flex items-center gap-2.5 px-3 py-2 text-xs font-bold text-slate-500 hover:text-wytnet-blue hover:bg-blue-50/20 rounded-xl transition-all"
                  >
                    <Icon className="h-3.5 w-3.5 text-slate-400 group-hover:text-wytnet-blue" />
        ...
        <div className="lg:col-span-9 space-y-6">
          ...
          <div className="flex items-center justify-between bg-white border border-slate-100 rounded-2xl px-5 py-3 shadow-[0_2px_8px_rgba(0,0,0,0.003)]">
            ...
            <div className="flex gap-2">
              ...
              <button className="flex items-center gap-1.5 bg-wytnet-blue hover:bg-wytnet-blue-light transition-all text-xs font-bold text-white px-3.5 py-1.5 rounded-lg cursor-pointer">
                  ...
                  {[25, 45, 60, 35, 55, 75, 85, 40, 50, 70, 30, 65, 20].map((h, i) => (
                    <div
                      ...
                      className={`w-[6%] rounded-t transition-all duration-300 ${
                        i % 3 === 0
                          ? 'bg-wytnet-blue/50 hover:bg-wytnet-blue'
                          : i % 3 === 1
                          ? 'bg-wytnet-blue/80 hover:bg-wytnet-blue'
                          : 'bg-wytnet-blue/30 hover:bg-wytnet-blue'
                      }`}
                      style={{ height: `${h}%` }}
                    />
                  ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {ssoCards.slice(0, 3).map((card) => (
              <Card
                key={card.id}
                className="border border-slate-100 bg-white rounded-2xl hover:border-wytnet-blue/20 hover:shadow-[0_8px_30px_rgba(0,102,204,0.015)] transition-all cursor-pointer shadow-[0_2px_8px_rgba(0,0,0,0.003)]"
              >
